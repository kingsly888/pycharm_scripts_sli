...
current_datetime = datetime.now(philippines_tz).strftime("%Y-%m-%d_%H-%M-%S")

# Define the file path with the current date and time
filename = f"Cluster_3_FS_Universe_{current_datetime}.csv"
download_path = os.path.expanduser(f"~/Downloads/{filename}")

# Save the DataFrame as CSV
df_c3_fs.to_csv(download_path, index=False)
